...
app.controller('indexController', function($scope, baseService,$http){
    ...
    $scope.findOrders = function () {
        ...
        // 发送异步请求
        baseService.sendPost("/order/findOrders" , $scope.searchParam).then(function(response){
    ...
    $scope.findOrder = function () {
        $http.get("/order/findCart", {"withCredentials":true}).then(function(response){
            // 获取响应数据 response.data: {total: 100, rows:[{},{}]} rows:List<SolrItem>
    ...
    /** 添加商品到购物车 */
    $scope.Cart = function(id,num){
        $http.get("order/cart?itemId="
            + id + "&num="
            + num)
            .then(function(response){
                if (response.data){
                    /** 跳转到购物车页面 */
                    location.href='http://cart.pinyougou.com/pay.html';
                }else{
                    alert("请求失败！");
                }
            });
